...
from matilda_env.db.sqlalchemy import api as db_session
#from css.i18n import _

INIT_VERSION = {}
INIT_VERSION['matilda_env'] = 0
_REPOSITORY = {}

LOG = logging.getLogger(__name__)


def get_engine(database='matilda_env', context=None):
    if database == 'matilda_env':
        return db_session.get_engine()


def db_sync(version=None, database='matilda_env', context=None):
...
def db_version(database='matilda_env', context=None):
    repository = _find_migrate_repo(database)
    try:
...
def db_version_control(version=None, database='matilda_env', context=None):
    repository = _find_migrate_repo(database)
...
def _find_migrate_repo(database='matilda_env'):
