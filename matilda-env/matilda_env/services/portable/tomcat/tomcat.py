...
def install_tomcat(host, username=None, password=None):
    ...
    stdout, stderr = transfer.execute_command('chmod 755 /tmp/*.sh', host, 22, username, password,
                                             change_user='root')
    stdout, stderr = transfer.execute_command('./tmp/tomcat_user_create.sh', host, 22, username, password,
                                              change_user='root')
    stdout, stderr = transfer.execute_command('sudo -u tomcat ./tmp/tomcat_install.sh', host, 22, username, password,
                                              change_user='root')
...
def deploy_application(warfile_location, host, username=None, password=None):
    transfer.sftp_file(warfile_location, warfile_location, '/home/tomcat/tomcat-7.0.85/webapps',
                           host, username, password)
    stdout, stderr = transfer.execute_command('sudo -u tomcat ./tmp/tomcat_restart.sh', host, 22, username, password,
                                              change_user='root')
